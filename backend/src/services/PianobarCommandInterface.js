    ...
    return this.retryHelper.retryOperation(
      async () => {
        try {
          ...
          // Always ensure permissions are correct - use 666 (read/write for all users)
          // Using 666 allows non-blocking writes when no reader is attached
          await execPromise(`chmod 666 ${this.pianobarCtl}`);
          logger.info(`Set FIFO permissions to 666 for ${this.pianobarCtl}`);
