...
@router.get(
    ...
    description="""
    Detailed system information for debugging and troubleshooting.
    
    **Used by:**
    - Developers debugging connection issues
    - System administrators checking configuration
    - Troubleshooting shade command failures
    
    **Returns:**
    - Arduino port and connection details
    ...
    responses={
        ...
        500: {"description": "Error retrieving system status"},
    }
)
async def system_status():
    """Get detailed system status for debugging"""
    try:
        logger.info("Retrieving detailed system status")
        
        status = await shade_service.get_system_status()
...
@router.get(
    ...
    description="""
    Ultra-simple ping endpoint that just confirms the service is responding.
    
    **Used by:**
    - Quick connectivity tests
    - Load balancer health checks that need minimal response
...
# Optional: Detailed Arduino connection test
@router.get(
    "/arduino",
    summary="Arduino connection test",
    description="""
    Test Arduino connection specifically.
    
    **Use this when:**
    - Shade commands are failing
    - Troubleshooting hardware connectivity
    - Verifying Arduino is responding to commands
    """,
    responses={
        200: {"description": "Arduino connection test results"},
    }
)
async def arduino_test():
    """Test Arduino connection specifically"""
    try:
        logger.info("Testing Arduino connection")
        
        # This will attempt to connect and send a test command
        from commander.interface.arduino_whisperer import arduino_connection
        ...
        if not currently_connected:
            connection_result = await arduino_connection.reconnect()
        else:
            connection_result = True
        
        return {
            "success": True,
            "arduino_connected": connection_result,
            "previously_connected": currently_connected,
            "port": status.get("port"),
            "recent_commands_count": 0,  # Command history not tracked in new system
            "last_command_time": status.get("last_successful_command"),
            "message": "Connected and responsive" if connection_result else "Connection failed"
        }
