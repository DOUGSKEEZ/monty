    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
      ...
      {awayContext.loading && (
        <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-md">
          <div className="flex items-center">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300">Loading periods...</p>
          </div>
        </div>
      )}
      ...
      {sortedPeriods.length > 0 ? (
            ...
            return (
              <div
                ...
                className={`p-3 rounded-lg border transition-all duration-200 ${
                  isCurrent
                    ? 'border-blue-400 bg-blue-50 dark:border-blue-600 dark:bg-blue-900/40'
                    : isPast
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      {isCurrent && <span className="text-blue-600 dark:text-blue-400 text-xs font-bold">CURRENT</span>}
                      {isPast && <span className="text-gray-500 dark:text-gray-400 text-xs">PAST</span>}
                    </div>

                    <p className={`font-medium ${
                      isCurrent ? 'text-blue-800 dark:text-blue-200' : isPast ? 'text-gray-600 dark:text-gray-300' : 'text-gray-800 dark:text-white'
                    }`}>
                      {formatDate(period.startDate)} - {formatDate(period.endDate)}
                    </p>

                    <p className={`text-sm ${
                      isCurrent ? 'text-blue-600 dark:text-blue-400' : isPast ? 'text-gray-500 dark:text-gray-400' : 'text-gray-600 dark:text-gray-300'
                    }`}>
                      {days} day{days !== 1 ? 's' : ''}
                    </p>
                  </div>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mb-1">No away periods scheduled</p>
          <p className="text-gray-400 dark:text-gray-500 text-xs">Use the date picker above to add periods</p>
        </div>
      ) : null}
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="bg-blue-50 dark:bg-blue-900/40 rounded-lg p-3">
              <p className="text-lg font-bold text-blue-800 dark:text-blue-200">{sortedPeriods.length}</p>
              <p className="text-xs text-blue-600 dark:text-blue-400">Total Periods</p>
            </div>
