...
# Fixed emergency restart script - enhanced version to fix server issues
...
HOST=0.0.0.0 npx nodemon src/server.js > /dev/null 2>&1 &
BACKEND_PID=$!
echo "Backend starting with nodemon, PID: $BACKEND_PID"
sleep 3
...
if lsof -i :3001 > /dev/null 2>&1; then
    ...
    cd /home/monty/monty/backend
    node direct-server.js > /dev/null 2>&1 &
    BACKEND_PID=$!
    ...
    if lsof -i :3001 > /dev/null 2>&1; then
        ...
        kill $BACKEND_PID
        HOST=0.0.0.0 node src/server.js > /dev/null 2>&1 &
        BACKEND_PID=$!
